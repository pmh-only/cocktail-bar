...
case "$PLUTO_CONNECTION" in
  AWS-VPC-GW1)
    ...
    PLUTO_MARK_IN_ARR=0x1
    PLUTO_MARK_OUT_ARR=0x1
    PLUTO_MARK_IN=0x1/0xffffffff
    ;;
  AWS-VPC-GW2)
    VTI_INTERFACE=vti2
    VTI_LOCALADDR=<Your Strongswan-CGW for VGW2 Inside CIDR>
    VTI_REMOTEADDR=<Your VGW2 Inside CIDR>
    PLUTO_MARK_IN_ARR=0x2
    PLUTO_MARK_OUT_ARR=0x2
    PLUTO_MARK_IN=0x2/0xffffffff
    ;;
esac
...
case "${PLUTO_VERB}" in
    up-client)
        ...
        $IP link add ${VTI_INTERFACE} type vti local ${PLUTO_ME} remote ${PLUTO_PEER} okey ${PLUTO_MARK_OUT_ARR} ikey ${PLUTO_MARK_IN_ARR}
