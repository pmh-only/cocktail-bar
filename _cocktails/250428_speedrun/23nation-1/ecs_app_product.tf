module "product" {
  ...
  container_definitions = {
    product = {
      essential = true
      image     = "648911607072.dkr.ecr.ap-northeast-2.amazonaws.com/product:latest"
      ...
      secrets = [{
        name      = "dbinfo"
        valueFrom = "arn:aws:secretsmanager:ap-northeast-2:648911607072:secret:dbsecret-VKd4yq"
      }]
      ...
      #   logDriver = "awslogs"
      #   options = {
      #     awslogs-group         = "/aws/ecs/${local.ecs_cluster_name}/project-product"
    ...
    log_router = {
      ...
      log_configuration = {
        logDriver = "awslogs"
        options = {
          awslogs-group         = "/wsi/webapp/product-logroute"
          awslogs-region        = var.region
          awslogs-stream-prefix = "ecs"
          awslogs-create-group  = "true"
        }
