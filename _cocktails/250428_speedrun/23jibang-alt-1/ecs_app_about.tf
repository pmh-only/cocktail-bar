module "ecs_service_about" {
  source = "terraform-aws-modules/ecs/aws//modules/service"

  force_new_deployment = true
  force_delete         = true

  name        = "${var.project_name}-about-svc"
  family      = "about-task-def"
  cluster_arn = module.ecs.cluster_arn

  security_group_name            = "${var.project_name}-about-sg"
  ...
  container_definitions = {
    myapp = {
      essential = true
      image     = "648911607072.dkr.ecr.ap-northeast-2.amazonaws.com/wsi-about:latest"
      ...
      log_configuration = {
        logDriver = "awslogs"
        options = {
          awslogs-group         = "/aws/ecs/${local.ecs_cluster_name}/about"
  ...
  load_balancer = {
    service = {
      target_group_arn = module.alb.target_groups.about.arn
...
resource "aws_cloudwatch_metric_alarm" "ecs_service_about_high" {
  alarm_name          = "ecs_service_about_high"
  ...
  metric_query {
    ...
    metric {
      ...
      dimensions = {
        ...
        ServiceName = module.ecs_service_about.name
      }
    }
  }
  ...
  alarm_actions = [module.ecs_service_about.autoscaling_policies.high.arn]
}


resource "aws_cloudwatch_metric_alarm" "ecs_service_about_low" {
  alarm_name          = "ecs_service_about_low"
  ...
  alarm_actions = [module.ecs_service_about.autoscaling_policies.low.arn]
}
