module "ecs_service_projects" {
  source = "terraform-aws-modules/ecs/aws//modules/service"

  force_new_deployment = true
  force_delete         = true

  name        = "${var.project_name}-projects-svc"
  family      = "projects-task-def"
  cluster_arn = module.ecs.cluster_arn

  security_group_name            = "${var.project_name}-projects-sg"
  ...
  container_definitions = {
    myapp = {
      essential = true
      image     = "648911607072.dkr.ecr.ap-northeast-2.amazonaws.com/wsi-projects:latest"
      ...
      log_configuration = {
        logDriver = "awslogs"
        options = {
          awslogs-group         = "/aws/ecs/${local.ecs_cluster_name}/projects"
  ...
  load_balancer = {
    service = {
      target_group_arn = module.alb.target_groups.projects.arn
...
resource "aws_cloudwatch_metric_alarm" "ecs_service_projects_high" {
  alarm_name          = "ecs_service_projects_high"
  ...
  metric_query {
    ...
    metric {
      ...
      dimensions = {
        ...
        ServiceName = module.ecs_service_projects.name
      }
    }
  }
  ...
  alarm_actions = [module.ecs_service_projects.autoscaling_policies.high.arn]
}


resource "aws_cloudwatch_metric_alarm" "ecs_service_projects_low" {
  alarm_name          = "ecs_service_projects_low"
  ...
  alarm_actions = [module.ecs_service_projects.autoscaling_policies.low.arn]
}
