locals {
  ...
  tgw_target_vpcs = [
    "vpc-0f09ca7fabaa3573e",
    "vpc-074ad470b0ca6fa81",
    "vpc-0a48f26d16b816325",
    "vpc-0b54458e305b78926",
  ]

  # Corresponding names for each TGW attachment
  # The index here must match the index of the VPC IDs above
  tgw_attachment_names = [
    "${var.project_name}-tgw-attach-app1",
    "${var.project_name}-tgw-attach-app2",
    "${var.project_name}-tgw-attach-egress",
    "${var.project_name}-tgw-attach-firewall",
  ]

  # Names for TGW route tables
  tgw_route_table_names = [
    "${var.project_name}-tgw-rtb-app1",
    "${var.project_name}-tgw-rtb-app2",
    "${var.project_name}-tgw-rtb-egress",
    "${var.project_name}-tgw-rtb-firewall",
  ]
}
