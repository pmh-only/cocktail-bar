...
locals {
  ...
  userdatas = {
    "al2" = <<-EOF
      ...
      baseurl=https://repo.mongodb.org/yum/amazon/2/mongodb-org/8.0/\$basearch/
      gpgcheck=1
      enabled=1
      gpgkey=https://www.mongodb.org/static/pgp/server-8.0.asc
      EOT

      yum install -y jq curl wget git docker mongodb-mongosh
      amazon-linux-extras install -y redis6 mariadb10.5 postgresql14 epel
    ...
    "al2023" = <<-EOF
      ...
      yum install -y --allowerasing jq curl wget git mariadb105 postgresql16 docker redis6 mongodb-mongosh
