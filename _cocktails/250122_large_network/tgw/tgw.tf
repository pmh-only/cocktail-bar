locals {
  tgw_name = var.project_name

  # VPCs for which we want TGW attachments
  tgw_target_vpcs = [
    "vpc-08a3465c1808993c0",
    "vpc-0ec8940207793467a",
    "vpc-0ddb253699d4f43f9"
  ]

  # Corresponding names for each TGW attachment
  # The index here must match the index of the VPC IDs above
  tgw_attachment_names = [
    "${var.project_name}-attach-main",
    "${var.project_name}-attach-spoke1",
    "${var.project_name}-attach-spoke2",
  ]

  # Names for TGW route tables
  tgw_route_table_names = [
    "${var.project_name}-rtb-main",
    "${var.project_name}-rtb-spoke1",
    "${var.project_name}-rtb-spoke2",
  ]
}
